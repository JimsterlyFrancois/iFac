...
router.get('/', authenticate, async (req, res) => {  
  try {  
    ...
    const documents = await Document.findForUser(req.user, limit, offset, search);  
    const total = await Document.countForUser(req.user, search);  
...
router.get('/:id', authenticate, async (req, res) => {  
  try {  
    ...
    // contrôle d'accès aligné sur findForUser  
    const isAdmin = req.user.is_admin === true;  
    const allowed =  
      isAdmin ||  
      (document.target_faculty === req.user.faculty &&  
        document.target_option === req.user.option &&  
        (document.target_level === req.user.level || document.target_level === 'ALL'));  
